...
import shap
import rasterio  # Use rasterio for TIFF images
from torch_optimizer import Lamb  # Import LAMB optimizer
...
# Step 5: SHAP Global Explanations with GradientExplainer
def shap_global_explanations(model, test_loader):
    model.eval()
    background = next(iter(test_loader))[0][:100].to(device)  # Use 100 samples as background
    test_images = next(iter(test_loader))[0][:5].to(device)  # Use 5 samples for explanation
