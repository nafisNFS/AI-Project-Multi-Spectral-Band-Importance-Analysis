...
class EuroSATDataset(Dataset):
    def __init__(self, dataset_path, band_indices, transform=None, label_map=None):
        self.dataset_path = dataset_path
        self.class_names = sorted(os.listdir(dataset_path))
        self.transform = transform
        self.band_indices = band_indices
        self.label_map = label_map  # Use the provided label map
        self.images = []
        self.labels = []
        ...
        for class_name in self.class_names:
            ...
            for image_name in os.listdir(class_path):
                ...
                with rasterio.open(image_path) as src:
                    ...
                    self.labels.append(self.label_map[class_name])  # Use label map for consistent indexing
...
def train_and_evaluate(band_combination_name, band_indices):
    ...
    dataset = EuroSATDataset(dataset_path, band_indices, transform=transform, label_map=label_map)
    ...
    for i, (prec, rec, f1_score_class) in enumerate(zip(classwise_precision, classwise_recall, classwise_f1)):
        class_name = list(label_map.keys())[i]  # Get the class name from the label map
        print(f"Class {class_name}: Precision: {prec:.4f}, Recall: {rec:.4f}, F1 Score: {f1_score_class:.4f}")
    ...
    return accuracy, precision, recall, f1, classwise_precision, classwise_recall, classwise_f1, shap_values, test_images, y_true
...
for band_combination_name, band_indices in band_combinations.items():
    accuracy, precision, recall, f1, classwise_precision, classwise_recall, classwise_f1, shap_values, test_images, y_true = train_and_evaluate(
    ...
    shap_outputs[band_combination_name] = (shap_values, test_images, y_true)  # Store SHAP outputs and true labels
...
# Display SHAP outputs for selected images from 4 classes
print("\nDisplaying SHAP outputs for selected images from 4 classes:")
for band_combination_name, (shap_values, test_images, y_true) in shap_outputs.items():
    print(f"\nSHAP Output for Band Combination: {band_combination_name}")

    # Select one image from each of the 4 classes
    class_names = ["AnnualCrop", "Forest", "Highway", "River"]  # Example class names
    selected_images = []
    selected_shap_values = []
    selected_labels = []

    for class_name in class_names:
        class_idx = list(label_map.keys()).index(class_name)  # Get the index of the class name
        # Find the first image from the test set that belongs to the selected class
        for i in range(len(test_images)):
            if y_true[i] == class_idx:
                selected_images.append(test_images[i])
                selected_shap_values.append(shap_values[class_idx][i])
                selected_labels.append(class_name)
                break

    # Convert selected images and SHAP values to numpy arrays
    selected_images = torch.stack(selected_images).cpu().numpy()
    selected_shap_values = [np.swapaxes(np.swapaxes(s, 1, -1), 1, 2) for s in selected_shap_values]

    # Plot SHAP values for the selected images
    shap.image_plot(selected_shap_values, -selected_images, labels=selected_labels)