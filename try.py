...
# Example mean and std values for 13 bands (replace with actual values)
mean = [0.5] * 13  # Placeholder mean for 13 bands
std = [0.5] * 13   # Placeholder std for 13 bands
...
class CustomCNN(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(13, 32, kernel_size=3, padding=1),  # 13 input channels (all bands)
            nn.GELU(),  # Use GELU
...
# Step 5: SHAP Global Explanations with KernelExplainer
def shap_global_explanations(model, test_loader):
    model.eval()

    # Use fewer samples for background and test images
    background = next(iter(test_loader))[0][:3].to(device)  # Use 3 samples as background
    ...
    background = resize_transform(background)
    test_images = resize_transform(test_images)

    # Use KernelExplainer
    def model_predict(x):
        # Reshape the flattened input back to the original shape
        x = x.reshape(-1, 13, 16, 16)  # Reshape to [num_samples, channels, height, width]
        x = torch.tensor(x, dtype=torch.float32).to(device)
        with torch.no_grad():
            return model(x).cpu().numpy()

    explainer = shap.KernelExplainer(model_predict, background.cpu().numpy().reshape(background.size(0), -1))
    shap_values = explainer.shap_values(test_images.cpu().numpy().reshape(test_images.size(0), -1))
    ...
    shap.summary_plot(shap_values, test_images.cpu().numpy().reshape(test_images.size(0), -1), feature_names=band_names, class_names=class_names, plot_type="bar")
